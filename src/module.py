...
class RNNLayer(nn.Module):
    ...
    def forward(self, input_x, x_len):
        ...
        # print(input_x.size(), " ", output.size())
...
class BaseAttention(nn.Module):
    ...
    def _attend(self, energy, value):
        ...
        self.mask = self.mask * (-np.inf) + 1
        attn = attn * self.mask
        
        attn = self.softmax(attn)  # BNxT
        output = torch.matmul(attn.unsqueeze(1), value).squeeze(
...
class ScaleDotAttention(BaseAttention):
    ...
    def forward(self, q, k, v):
        ts = k.shape[1]
        energy = torch.matmul(q.unsqueeze(1), k.transpose(
