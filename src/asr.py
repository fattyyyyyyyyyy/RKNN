...
class ASR(nn.Module):
    ...
    def forward(self, audio_feature, feature_len=298, decode_step=20, tf_rate=0.0, teacher=None,
        ...
        l = audio_feature.shape[1]
        c = audio_feature.shape[2]
        audio_feature = torch.reshape(audio_feature, (bs, 398, 120))
        ctc_output, att_output, att_seq = None, None, None
        dec_state = [] if get_dec_state else None
        
        feature_len = torch.ones(1, dtype=int)*feature_len

        # Encode
        encode_feature, encode_len = self.encoder(audio_feature, feature_len)
        # encode_feature = torch.ones(1,74,1024)
        # encode_len = torch.ones(1)
        ...
        att_output = torch.zeros(1)
        att_seq = torch.zeros(1)
...
class Encoder(nn.Module):
    ...
    def forward(self, input_x, enc_len):
        for i, layer in enumerate(self.layers):
            ...
            # print(i, "  ", input_x.size())
        return input_x, enc_len
    
if __name__ == "__main__":
    config = yaml.load(open('config/libri/asr_example.yaml', 'r'), Loader=yaml.FullLoader)
    feat_dim = 120
    vocab_size = 16000
    ...
    inp = torch.randn(4, 207, 120)
    inp_len = torch.ones(4, dtype=int) * 207
    txt = torch.randint(0, 100, (4, 20))
    txt_len = torch.ones(4, dtype=int) * 20
    
    ctc_output, encode_len, att_output, att_align, dec_state = model(inp, inp_len, int(max(txt_len)))
    att_wer = cal_er(tokenizer, att_output, txt)
    ctc_wer = cal_er(tokenizer, ctc_output, txt, ctc=True)
    ctc_out = tokenizer.decode(ctc_output[0].argmax(dim=-1).tolist())
    att_out = tokenizer.decode(att_output[0].argmax(dim=-1).tolist())
